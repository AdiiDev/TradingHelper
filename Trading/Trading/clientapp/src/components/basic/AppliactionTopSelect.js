...
const ApplicationTopSelect = () => {
  ...
  const brokerAccountData = useSelector((state) => state.brokerAccounts.brokerAccounts)
  const brokerSelectedAccount = useSelector((state) => state.brokerAccounts.selectedBroker)
  const layouts = useSelector((state) => state.layouts.layouts)
  const selectedLayout = useSelector((state) => state.layouts.selectedLayout)
  const showSelectedLayout = useSelector((state) => state.layouts.showLayoutsSelect)
            ...
            {brokerAccountData.length > 0 ? (
              <Box sx={{ minWidth: 180 }}>
                <FormControl variant="filled" fullWidth>
                  ...
                  >
                    {brokerAccountData.map((account) => <MenuItem key={"ac" + account.id} value={account.id}>{account.brokerName}</MenuItem>)}
                  </Select>
                </FormControl>
              </Box>
            ) : (
            ...
            {showSelectedLayout &&
              <Box sx={{ minWidth: 180 }}>
                <FormControl variant="filled" fullWidth>
                  ...
                  >
                    <MenuItem key={"la"} value={0}>Sandbox</MenuItem>
                    {layouts.map((lay) => <MenuItem key={"la" + lay.id} value={lay.id}>{lay.name}</MenuItem>)}
                  </Select>
                </FormControl>
              </Box>
            }
