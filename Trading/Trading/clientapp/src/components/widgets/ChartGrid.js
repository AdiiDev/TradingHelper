...
const ChartGrid = ({
    ...
    if (chartsInfo.intervals.length > 1) {
      for (let i = 0; i < chartsInfo.intervals.length; i++)
        newArray.push({
          row: chartsInfo.row,
          column: i,
          symbol: chartsInfo.symbol,
          interval: chartsInfo.intervals[i],
        })
    } else {
      newArray.push({
        row: chartsInfo.row,
        column: chartsInfo.column,
        symbol: chartsInfo.symbol,
        interval: chartsInfo.intervals[0],
      })
    }
  ...
  const removeChart = (rowId, columnId) => {
    const newArray = currentWidgetsArray.filter(
      (x) => !(x.row === rowId && x.column === columnId)
    )
    setCurrentWidgetsArray(newArray)
  }
  ...
  const columns =
    selectedLayout !== null ? selectedLayout.columns : settings.columns
      ...
      {Array.from({ length: rows }, (_, rowIndex) => (
        <Grid container spacing={2} key={`row-${rowIndex}`}>
          {Array.from({ length: columns }, (_, columnIndex) => {
            const isInArray = currentWidgetsArray.find(
              (element) =>
                element.row === rowIndex && element.column === columnIndex
            )
            const vh = Math.max(
            ...
            return (
              <Chart
                ...
                symbol={isInArray.symbol}
                interval={isInArray.interval}
